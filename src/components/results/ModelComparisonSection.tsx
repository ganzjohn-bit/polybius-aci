...
import { getClusterAverages } from '@/lib/model-utils';
import { clusterLabels } from '@/data/constants';
...
export default function ModelComparisonSection({
  ...
  const clusterAverages: ClusterAverage[] = getClusterAverages(modelScores, clusterLabels);

  return (
    <Card variant="section" className="mb-10 bg-slate-50 border-slate-200" icon={BookOpen} title="Theoretical Model Assessment">
      <p className="text-slate-600 text-sm mb-4">
        Each model applies its own theoretical weights to the shared factor scores, producing different risk assessments. Models flagged as outliers deviate &gt;1 standard deviation from the mean (35.5).
      </p>
